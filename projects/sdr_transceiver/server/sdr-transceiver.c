...
int main(int argc, char *argv[])
{
  ...
  switch(number)
  {
    ...
    case 2:
      port = 1002;
      cfg = mmap(NULL, sysconf(_SC_PAGESIZE), PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0x40005000);
      sts = mmap(NULL, sysconf(_SC_PAGESIZE), PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0x40006000);
  ...
  *rx_freq = (uint32_t)floor(600000/100.0e6*(1<<30)+0.5);
  *rx_sync = 0;
  /* set default rx sample rate */
  *rx_rate = 500;

  /* set default tx phase increment */
  *tx_freq = (uint32_t)floor(600000/100.0e6*(1<<30)+0.5);
  *tx_sync = 0;
  ...
  while(1)
  {
    ...
    switch(command >> 28)
    {
      case 0:
        ...
        *rx_freq = (uint32_t)floor(freq/100.0e6*(1<<30)+0.5);
        *rx_sync = freq > 0 ? 0 : 1;
        break;
      case 1:
        /* set rx sample rate */
  ...
  while(1)
  {
    ...
    switch(command >> 28)
    {
      case 0:
        ...
        freq = command & 0xfffffff;
        if(freq < freq_min || freq > freq_max) continue;
        *tx_freq = (uint32_t)floor(freq/100.0e6*(1<<30)+0.5);
        *tx_sync = freq > 0 ? 0 : 1;
        break;
      case 1:
        /* set tx sample rate */
